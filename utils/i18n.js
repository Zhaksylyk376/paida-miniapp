...
const dict = {
  zh: {
    ...
    order_border: '口岸（过境点）',
    order_border_ph: '选择口岸（可选，用于估算排队时间）',
    order_queue: '海关排队',
    order_hours: '小时',
    order_km: '公里',
    order_distance: '距离',
    order_drive_days: '行驶',
    order_eta_total: '综合到达估算',
    order_route_partial: '未收录该城市，仅显示口岸排队时间。您仍可下单。',
    order_goods: '货物类型',
  ...
  ru: {
    ...
    order_border_ph: 'Выберите пункт (не обязательно, для оценки очереди)',
    order_queue: 'Очередь на таможне',
    order_hours: 'ч',
    order_km: 'км',
    order_distance: 'Расстояние',
    order_drive_days: 'В пути',
    order_eta_total: 'Итоговое ETA',
    order_route_partial: 'Города нет в справочнике — показана только оценка по очереди. Заказ можно оформить.',
